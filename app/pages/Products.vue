<template>
  ...
  <div class="container mx-auto max-w-7xl px-6 py-8 space-y-8">
    ...
    <div
      v-if="aiResults && aiResults.length && !hasActiveFilters"
      class="-mt-4"
    >
      <span
        class="inline-flex items-center rounded-full border px-3 py-1 text-xs text-blue-700"
        >AI results</span
      >
    </div>
    ...
    <div v-else-if="!loading" class="text-sm text-gray-500">
      Nothing matches your search, you can use our AI assistant to find the best
      products for you.
    </div>
...
const baseResults = computed(() => {
  if (aiResults.value?.length && !hasActiveFilters.value)
    return aiResults.value as any[];
...
const visible = computed(() => {
  const out = baseResults.value.filter((p: any) => {
    ...
    const normalized = (p.tags ?? []).map((t: string) =>
      t.trim().toLowerCase()
    );
    const tagsOk =
      filters.tags.length === 0 ||
      filters.tags.some((t: string) =>
        normalized.includes(t.trim().toLowerCase())
      );
